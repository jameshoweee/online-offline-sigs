...
/* Repetitions, each with FRESH randomness (new seed -> new key + message +
 * offline token) per iteration; host aggregates median over all of them.
 * The paper uses ~1000; 100 keeps the M7 UART run to about a minute. */
#ifndef OO_ITER
#define OO_ITER 100
#endif

/* hal_checkstack() hangs on the F767's split RAM map; the M4 stack figures are
 * already recorded, and the M7 comparison to the paper is about cycles. Set to
 * 1 to re-enable per-op stack measurement (works on the flat-SRAM M4). */
#ifndef STACK_MEAS
#define STACK_MEAS 0
#endif
#define LOGN 9
